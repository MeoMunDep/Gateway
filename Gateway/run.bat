@echo off
title Gateway Bot by @MeoMunDep
color 0A
...
cls
echo =================================================================
echo    Gateway BOT SETUP AND RUN SCRIPT by @MeoMunDep
echo =================================================================
echo.
...
if exist "..\node_modules" (
    ...
    npm install user-agents axios colors p-limit https-proxy-agent socks-proxy-agent web3 ethers
    cd %~dp0
) else (
...
if not exist configs.json (
    ...
    echo   "referralCode": "M1XYTDLPYK">> configs.json
    echo }>> configs.json
